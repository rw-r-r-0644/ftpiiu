...
static int mount_fs(void *pClient, void *pCmd, char **mount_path)
{
    int result = -1;
...
static int unmount_fs(void *pClient, void *pCmd, const char *mountPath)
{
    int result = -1;
...
int mount_sd(const char *path)
{
    ...
    if(mount_fs(pClient, pCmd, &mountPath) == 0) {
...
int unmount_sd(const char *path)
{
    ...
    if(result == 0)
    {
        unmount_fs(pClient, pCmd, mountPath);
